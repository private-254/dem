...
let trashplug = async (m, { text,reply,davetech,prefix,command }) => {
    ...
    fetch(`https://api.tiklydown.eu.org/api/download/v5?url=${encodeURIComponent(text)}`)
        ...
        const title = `*Downloaded by ${botname}*`
            ...
            davetech.sendMessage(m.chat, { caption: title, video: { url: videoUrl }}, { quoted: m });
            davetech.sendMessage(m.chat, { audio: { url: audioUrl }, mimetype: 'audio/mp4' }, { quoted: null });
        })
