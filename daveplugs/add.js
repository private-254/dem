...
let trashplug = async (m, {command, prefix,isAdmins,trashown,reply,text,davetech }) => {
				...
				if (!text && !m.quoted) {
					...
					try {
						await davetech.groupParticipantsUpdate(m.chat, [numbersOnly], 'add')
							.then(async (res) => {
								for (let i of res) {
									let invv = await davetech.groupInviteCode(m.chat);
									...
									if (i.status == 403) {
										...
										await davetech.sendMessage(`${numbersOnly ? numbersOnly : creator}`, { 
