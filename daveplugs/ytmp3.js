...
let trashplug = async (m, { davetech, args, command,reply }) => {
    try {
        ...
        await davetech.sendMessage(m.chat, { document: { url: downloadURL }, fileName: filename, mimetype: format === 'mp4' ? 'video/mp4' : 'audio/mp3' }, { quoted: m })      
    } catch (e) {
        reply(e.message)
    }
}
