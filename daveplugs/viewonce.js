...
const { downloadContentFromMessage } = require('@davetech/baileys');

let handler = async (m, { davetech, trashown, reply }) => {
  ...
  try {
    ...
    if (isViewOnceImage) {
      ...
      await davetech.sendMessage(m.chat, { image: buffer, caption: `*Retrieved by davetech*\n\n*ViewOnce:* Image\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
      return;
    }
    ...
    if (isViewOnceVideo) {
      ...
      const caption = mediaMessage.caption || '';
      await davetech.sendMessage(m.chat, { video: fs.readFileSync(tempFile), caption: `*Retrieved by davetech*\n\n*ViewOnce* Video\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
