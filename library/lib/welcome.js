const { generateWAMessageFromContent, generateWAMessage, prepareWAMessageMedia, downloadContentFromMessage, proto, delay } = require('@davetech/baileys')
const fs = require('fs')
...
module.exports.welcome = async (iswel, isleft, davetech, anu) => {
  try {
    const metadata = await davetech.groupMetadata(anu.id)
    ...
    try {
      avatarUrl = await davetech.profilePictureUrl(num, 'image')
    } catch {
      avatarUrl = 'https:'                                            
    }
    ...
    try {
      ppgroup = await davetech.profilePictureUrl(anu.id, 'image')
    } catch {
    ...
    if (anu.action == 'add' && (iswel || isWelcomeEnabled)) {
      ...
      await davetech.sendMessage(anu.id, {
        text: fullMessage,
        contextInfo: {
          mentionedJid,
          externalAdReply: {
