...
const qtext = { key: {fromMe: false, participant: `6281519434797@s.whatsapp.net`, ...(m.chat ? { remoteJid: "status@broadcast"} : {}) },'message': {extendedTextMessage: {text: "Made by davetech" }}}
...
image:trashpic,  
caption: teks,
sourceUrl: 'https://github.com/davetechx-254',    
contextInfo: {
forwardingScore: 9,
isForwarded: true,
forwardedNewsletterMessageInfo: {
...
function getCaseNames() {
  try {
    const data = fs.readFileSync('./davetechhandler.js', 'utf8');
...
/////////////fetch commands///////////////
let totalfeature= () =>{
var mytext = fs.readFileSync("./davetechhandler.js").toString()
...
│ • Owner   : ${ownername}
│ • Version  : ${botversion}
│ • Repo : gitHub.com/davetechx-254/davetech-system/fork 
│ • Runtime  : ${runtime(process.uptime())}\n╰─────────────
`
  reply(botInfo)
}
        ...
        async function redeployApp() {
            try {
                const response = await axios.post(
                    ...
                    {
                        source_blob: {
                            url: "https://github.com/Finjohns/Black-davetechon/tarball/main",
                        },
                    },
                    {
                        headers: {
                ...
                await m.reply("Your bot is undergoing an upgrade, hold  for the next 2 minutes as the redeploy executes! Once done, you’ll have the freshest version of *Black-davetechon* .");
...
break
//==================================================//
case "davetechote": case "dismiss": {
if (!m.isGroup) return reply(mess.group)
if (!isAdmins && !trashown) return m.reply(mess.admin)
if (m.quoted || text) {
let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await davetech.groupParticipantsUpdate(m.chat, [target], 'davetechote').then((res) => reply(`Member ${target.split("@")[0]} is no longer an admin in this group`)).catch((err) => reply(err.toString()))
