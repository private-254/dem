        ...
        try {
            ...
            for (const number of numbers) {
                try {
                    ...
                    if (!exists) {
                        results.push(`⚠️ ${cleanNumber} - Not on WhatsApp`);
                        failCount++;
                        continue;
                    }
...
},
{
    ...
    async execute(sock, m, args, context) {
        ...
        try {
            ...
            await context.reply({ text: `Failed to close group: ${error.message}` }, { quoted: m });
        }
    }
},
{
    ...
    async execute(sock, m, args, context) {
        ...
        try {
            ...
            await context.reply({ text: `Failed to open group: ${error.message}` }, { quoted: m });
        }
    }
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isOwner) {
            if (!isBotAdmin) {
                return await reply('Bot needs admin permissions.');
            }
            if (!isSenderAdmin) {
                return await reply('Only admins can remove members.');
            }
        }
        ...
        if (usersToKick.length === 0) {
            return await reply('Mention user or reply to their message!');
        }

        const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
        if (usersToKick.includes(botId)) {
            return await reply("Can't remove myself!");
        }
        ...
        try {
            ...
            await reply('Failed to remove user(s)!');
        }
    }
},
{
    ...
    async execute(sock, m, args, context) {
        ...
        try {
            ...
            if (!isOwner && !isAdmin) {
                return await context.reply({ text: "Only admins can get invite link!" }, { quoted: m });
            }
            ...
            await context.reply({ text: `Failed to get link: ${error.message}` }, { quoted: m });
        }
    }
},
{
    ...
    async execute(sock, m, args, context) {
        try {
            ...
            await context.reply({ text: 'Failed to get group info.' }, { quoted: m });
        }
    }
},
{
    ...
    execute: async (sock, m, args, context) => {
        try {
            ...
            try {
                ...
                const demoteMessage = `📋 Admin Demoted\n\nGroup: ${groupName}\nUser: @${userName}\nRole: Member`;
                ...
                if (demoteError.message.includes('forbidden')) {
                    errorMsg = 'No permission to demote.';
                } else if (demoteError.message.includes('not-authorized')) {
                    errorMsg = 'Need admin privileges.';
                }
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isSenderAdmin) {
            return await reply('Only admins can promote members.');
        }
        ...
        if (userToPromote.length === 0) {
            return await reply('Mention or reply to user to promote!');
        }
        ...
        try {
            ...
            for (const user of userToPromote) {
                ...
                const promotionMessage = `📋 Admin Promoted\n\nGroup: ${groupName}\nUser: @${userName}\nRole: Admin`;
...
},
{
    ...
    async execute(sock, m, args, context) {
        ...
        try {
            ...
            await context.reply({ text: `Error: ${error.message}` }, { quoted: m });
        }
    }
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isSenderAdmin) {
            return await reply('Only admins can mute group.');
        }
        ...
        try {
            ...
            await reply('Failed to mute/unmute group.');
        }
    }
}, 
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isSenderAdmin) {
            return await reply('Only admins can unmute group.');
        }
    ...
    async execute(sock, m, args, context) {
        ...
        try {
            ...
            if (action === 'tag') {
                ...
                await reply(`👑 Admins (${uniqueAdmins.length})\n\n${adminTags}`, { mentions });

            } else {
                let adminList = `👑 Admins (${uniqueAdmins.length})\n\n`;

                uniqueAdmins.forEach((admin, index) => {
                    const displayName = getDisplayName(admin);
                    let role = (admin.id === groupOwner) ? '👑 Owner' : '⭐ Admin';
                    adminList += `${index + 1}. ${role}: @${displayName}\n`;
                });

                adminList += `\nUse .admins tag to mention all`;
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isSenderAdmin && !isBotAdmin) {
            return await reply('Only admins can use tagall.');
        }
        if (!senderIsSudo){
            return await reply('Only owner can use tagall.');
        }
        ...
        try {
            ...
            if (!participants || participants.length === 0) {
                return await reply('No members found.');
            }

            const customMessage = rawText.split(' ').slice(1).join(' ').trim();
            let message = customMessage ? `📢 ${customMessage}` : '📢 All Members:';
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        try {
            ...
            if (!isSenderAdmin) {
                await react('🚫');
                return await reply('Only admins can use this.');
            }
            ...
            if (nonAdmins.length === 0) {
                await react('ℹ️');
                return await reply('No non-admin members.');
            }

            let text = '📢 Non-Admin Members:\n\n';
...
},
{
    ...
    aliases: [],
    category: 'GROUP',
    description: 'Tag all admin members',
    usage: '.tagadmin',
    ...
    execute: async (sock, message, args, context) => {
        ...
        try {
            ...
            if (admins.length === 0) {
                await react('ℹ️');
                return await reply('No admin members.');
            }

            let text = '📢 Admin Members:\n\n';
            admins.forEach(p => {
                text += `@${p.id.split('@')[0]}\n`;
            });
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        const { isSenderAdmin, isBotAdmin, chatId, reply, react, isGroup } = context;
        ...
        if (!imageMessage) {
            return await reply('Reply to an image or sticker.');
        }
        ...
        try {
            ...
            await react('❌');
            await reply('Failed to update photo.');
        }
    }
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        try {
            ...
            await react('❌');
            await reply('Failed to update name.');
        }
    }
},
{
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        const { chatId, reply, isSenderAdmin, isBotAdmin, react, isGroup } = context;
...
},
{
    ...
    execute: async (sock, message, args, context) => {
        ...
        if (!isSenderAdmin) {
            await react('🚫');
            return await reply('Only admins can create polls.');
        }
        ...
        if (!questionPart || !optionsPart) {
            return await reply('Format: .poll "Question?" | Option1, Option2');
        }
