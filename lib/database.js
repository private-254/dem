...
const DEFAULT_DATABASE = {
    settings: {
        ...
        autoemoji: "off",
        autorecord: false,
        autoviewstatus: true,  // Changed to true (default ON)
        autoreactstatus: false, // Renamed from autostatusreact for consistency
        autostatusreact: false, // Keep both for compatibility
        autorecordtype: "off",
    ...
    commandData: {
        ...
        antidelete: {}, // Anti-delete settings per chat
...
const loadDatabase = () => {
    try {
        ...
        // Ensure new settings exist in old databases
        if (!mergedDb.settings.autoviewstatus) mergedDb.settings.autoviewstatus = true;
        if (!mergedDb.settings.autostatusreact) mergedDb.settings.autostatusreact = false;
        if (!mergedDb.settings.statusEmojis) mergedDb.settings.statusEmojis = DEFAULT_DATABASE.settings.statusEmojis;
        if (!mergedDb.settings.reactionEmojis) mergedDb.settings.reactionEmojis = DEFAULT_DATABASE.settings.reactionEmojis;
        if (!mergedDb.settings.autoemojiList) mergedDb.settings.autoemojiList = DEFAULT_DATABASE.settings.autoemojiList;
 ...
 */
const setAntibadword = (chatId, enabled) => updateCommandData('antibadword', chatId, enabled);
const getAntibadword = (chatId) => getCommandData('antibadword', chatId, false);
const isAntibadwordEnabled = (chatId) => getAntibadword(chatId) === true;

/**
 * Antidelete functions
 */
const setAntidelete = (chatId, enabled) => updateChatData(chatId, 'antidelete', enabled);
const getAntidelete = (chatId) => getChatData(chatId, 'antidelete', false);
const isAntideleteEnabled = (chatId) => getAntidelete(chatId) === true;
